...
function buildInitialForm(event) {
  return {
    ...
    startDateTime: toLocalDateTimeString(new Date(event.startDateTime)).slice(
      0,
      16,
    ),
    endDateTime:
      event.type === "RADUNO"
        ? toLocalDateTimeString(new Date(event.endDateTime)).slice(0, 16)
        : "",
    routeId: event.route?.id || "",
    meetingPointLat: event.route ? null : event.meetingPointLat,
    meetingPointLng: event.route ? null : event.meetingPointLng,
    meetingPointLabel: event.route ? "" : event.meetingPointAddress || "",
    bufferMinutes: 0,
...
function EventEditPage() {
  ...
  if (!event.organizer) {
    return (
      <div className="empty-state empty-state-margin">
        Non sei l'organizzatore di questo evento.
      </div>
    )
  }
  ...
  if (event.parentEventId) {
    return (
      <div className="empty-state empty-state-margin">
        I singoli giorni di un viaggio non sono ancora modificabili da qui.
      </div>
    )
  }
  ...
  const estimatedEndDateTime =
    event.type === "STANDARD" && selectedRoute && form.startDateTime
          ...
          <>
            <div>
              <div className="field-label form-group__label">
                PERCORSO {event.type === "RADUNO" ? "(OPZIONALE)" : ""}
              </div>
              {isLoadingRoutes ? (
                <Spinner
                  size="sm"
              ...
              {event.type === "RADUNO" && !form.routeId && (
        ...
        {event.visibility !== "INVITE_ONLY" && (
          <div>
            ...
            <input
              type="number"
              className="input"
              min={event.currentParticipants || 1}
